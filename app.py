...
def generate_plots(N, mu, sigma2, S):
    ...
    Y = np.random.normal(mu, np.sqrt(sigma2), size=(N, 1))  # Replace with code to generate random values for Y with specified mean and variance
    ...
    for _ in range(S):
        ...
        Y_sim = np.random.normal(mu, np.sqrt(sigma2), size=(N, 1))  # Replace with code to generate Y values
